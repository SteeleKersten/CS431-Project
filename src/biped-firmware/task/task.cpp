...
bestEffortTask()
{
    ...
    //    actuation_command.motor_left_pwm = 100.0;  // Above minimum PWM (0)
    //    actuation_command.motor_right_pwm = 100.0; // Above minimum PWM (0)
    //
...
networkTask(void* pvParameters)
{
     ...
     *  TODO LAB 5 YOUR CODE HERE.
     */
    while (wifi_ == nullptr || wifi_->getWiFiStatus() != 3)
    {
...
realTimeTask(void* pvParameters)
{
    ...
    for (;;)
    {
         ...
         *  TODO LAB 7 YOUR CODE HERE.
         */
        if (controller_ != nullptr && actuator_ != nullptr)
        {
            ActuationCommand actuation_command = controller_->getActuationCommand();
            actuator_->actuate(actuation_command);
        }
        ...
        execution_time_real_time_task_ = time_point_current - time_point_start;
    }
...
udpReadBipedMessageTask(void* pvParameters)
{
    ...
    for (;;)
    {
         ...
         *  TODO LAB 5 YOUR CODE HERE.
         */
        if (wifi_ == nullptr || wifi_->getWiFiStatus() != 3)
        {
            vTaskDelay(100 / portTICK_PERIOD_MS);
            continue;
        }
