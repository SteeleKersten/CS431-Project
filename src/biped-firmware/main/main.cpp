...
setup()
{
     ...
     *  written only to the first byte from its starting address.
     *
     ...
     *  header for the Biped serial number EEtd::shared_ptr<Maneuver> custom_1 = std::make_shared<Maneuver>();
...
//    custom_5->next = nullptr;PROM address.
     *
     *  TODO LAB 1 YOUR CODE HERE.
     */
     ...
     *  std::make_shared function.
     *
     *  Note that the Biped firmware UDP class has identical name to the Arduino UDP
     *  class. Since we are currently in the global namespace and outside Biped firmware
     *  namespaces, therefore, to avoid collision, explicitly resolve the namespaces by
     *  using the Biped firmware UDP class as biped::firmware::UDP.
     *
     *  Refer to the global header for the global object shared pointers.
     *
...
//    planner_ = std::make_shared<WaypointPlanner>();
     ...
     *
     *  Refer to the Biped project task header for the task functions, the global header
     *  for the task handle pointers and the parameter header for the task parameters,
     *  such as the stack size, priority, CPU core, etc.
     *
     *  TODO LAB 3 YOUR CODE HERE.
     */
    xTaskCreatePinnedToCore(ioExpanderAInterruptServiceTask, "ioExpanderAInterruptServiceTask",
            TaskParameter::stack_size, nullptr, TaskParameter::priority_max,
            &task_handle_io_expander_a_interrupt_service_, TaskParameter::core_1);
    xTaskCreatePinnedToCore(ioExpanderBInterruptServiceTask, "ioExpanderBInterruptServiceTask",
            TaskParameter::stack_size, nullptr, TaskParameter::priority_max,
            &task_handle_io_expander_b_interrupt_service_, TaskParameter::core_1);
     ...
     *  TODO LAB 3 YOUR CODE HERE.
     */
    xTaskCreatePinnedToCore(realTimeTask, "realTimeTask", TaskParameter::stack_size, nullptr,
            TaskParameter::priority_max - 1, &task_handle_real_time_, TaskParameter::core_1);
    xTaskCreatePinnedToCore(udpReadBipedMessageTask, "udpReadBipedMessageTask",
            TaskParameter::stack_size, nullptr, TaskParameter::priority_min,
            &task_handle_udp_read_biped_message_, TaskParameter::core_1);
    ...
    {
        biped::firmware::Serial(LogLevel::warn) << "Initialized with error(s)";
    }
    else
    {
